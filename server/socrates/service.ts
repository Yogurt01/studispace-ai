...
export class SocratesService {
  ...
  async respond(input: { threadId: string; userId: string; message: string; mode: SocratesMode; context?: string; provider?: ModelProviderId }) {
    const previous = await this.repo.load(input.threadId, input.userId);
    // Provider is chosen per turn, so a thread can switch models without losing its history.
    const provider = this.router.resolve(input.provider);
    const now = Date.now();
    ...
    const state: ConversationState = { threadId: input.threadId, userId: input.userId, mode: input.mode, messages: [...(previous?.messages || []), userMessage].slice(-20), currentMessage: input.message, context: input.context, provider: provider.id, updatedAt: now };
    ...
    return { reply, mode: input.mode, threadId: input.threadId, provider: provider.id };
  }
}
